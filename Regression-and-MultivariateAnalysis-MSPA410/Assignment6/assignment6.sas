...
/*MSE & MAE for test data model*/
/*
data model1_output; set model1_output;
Title 'Model_Test';
if train=1 then do;
...
square_error = (response_VV-yhat)*(response_VV-yhat); end;

proc means data=model1_output nway noprint;
class train;
var square_error absolute_error;
output out = train_error 
	mean(square_error)=MSE_1
	mean(absolute_error)=MAE_1;
run; quit;

proc print data = train_error; run; quit;
*/
/*MSE & MAE for all data model*/
/*
data model1all_output; set model1all_output;
Title 'Model_All';
if train=1 then do;
absolute_error = abs(resid);
square_error = resid*resid; end;
